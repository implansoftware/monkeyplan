import {
  ...
  documents, Document, InsertDocument, DocumentType, DocumentSourceType
} from "@shared/schema";
...
export interface IStorage {
  ...
  updateUser(id: string, updates: Partial<Pick<User, 'username' | 'email' | 'fullName' | 'role' | 'isActive' | 'repairCenterId' | 'resellerId' | 'resellerCategory' | 'password' | 'phone' | 'partitaIva' | 'ragioneSociale' | 'codiceFiscale' | 'indirizzo' | 'citta' | 'cap' | 'provincia' | 'pec' | 'codiceUnivoco' | 'logoUrl' | 'hasAutonomousInvoicing'>>): Promise<User>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateUser(id: string, updates: Partial<Pick<User, 'username' | 'email' | 'fullName' | 'role' | 'isActive' | 'repairCenterId' | 'resellerId' | 'resellerCategory' | 'password' | 'phone' | 'partitaIva' | 'ragioneSociale' | 'codiceFiscale' | 'indirizzo' | 'citta' | 'cap' | 'provincia' | 'pec' | 'codiceUnivoco' | 'logoUrl' | 'hasAutonomousInvoicing'>>): Promise<User> {
  ...
  async getAllOpenPosSessions(repairCenterId: string): Promise<PosSession[]> {
  ...
  async listDocuments(filters: {
    ...
    if (filters.resellerId) conditions.push(eq(documents.resellerId, filters.resellerId));
    if (filters.repairCenterId) conditions.push(eq(documents.repairCenterId, filters.repairCenterId));
    ...
    if (filters.sourceId) conditions.push(eq(documents.sourceId, filters.sourceId));
    if (filters.startDate) conditions.push(gte(documents.createdAt, filters.startDate));
    if (filters.endDate) conditions.push(lte(documents.createdAt, filters.endDate));
    if (filters.search) {
      conditions.push(or(
        ilike(documents.title, `%${filters.search}%`),
        ilike(documents.fileName, `%${filters.search}%`),
        ilike(documents.sourceReference, `%${filters.search}%`)
      )!);
    }
    ...
    return { documents: docResults, total: totalResult?.count || 0 };
  }

  async listDocumentsForReseller(resellerId: string, filters: {
    documentType?: DocumentType;
    sourceType?: DocumentSourceType;
    search?: string;
    startDate?: Date;
    ...
    const conditions: SQL[] = [
      or(
        inArray(documents.resellerId, allResellerIds),
        ...(centerIds.length > 0 ? [inArray(documents.repairCenterId, centerIds)] : [])
      )!
    ];
  ...
  async listDocumentsForRepairCenter(repairCenterId: string, filters: {
    documentType?: DocumentType;
    sourceType?: DocumentSourceType;
    search?: string;
    startDate?: Date;
    ...
    const conditions: SQL[] = [eq(documents.repairCenterId, repairCenterId)];
  ...
  async getDocumentsBySource(sourceType: DocumentSourceType, sourceId: string): Promise<Document[]> {
