...
*   **User Management**: Backend API for listing and updating users, with specialized endpoints for resellers and customer registration. Includes tenant ownership tracking via `resellerId`.
    ...
    *   **Reseller Repair Center Management**: Resellers can manage their own repair centers from `/reseller/repair-centers` page with full CRUD operations. Includes "Usa i miei dati fiscali" checkbox that auto-fills fiscal data (ragioneSociale, partitaIva, codiceFiscale, IBAN, codiceUnivoco, PEC, address) from the reseller's profile when creating a new center.
