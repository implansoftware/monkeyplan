...
*   **Business Customer Sub-Clients**: Customers with Partita IVA can manage their own end-clients (via `customerBranches` table, reusing existing FRANCHISING/GDO structure). Sub-clients appear as a dropdown when creating remote repair requests (`branchId` field on `remoteRepairRequests`). The "I miei clienti" section is visible in the customer profile only for business customers. Remote request lists show the associated sub-client name. Backend enrich `GET /api/customer/remote-requests` and `GET /api/reseller/remote-requests` with `branchName` and `branchContactName`.
