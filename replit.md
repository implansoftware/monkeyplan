...
### Centralized Documents Section
- Added `documents` table to track all PDFs generated by the platform
- Document types: intake, delivery, diagnosis, quote, label, invoice, receipt, shipping, attachment, contract, report, other
- Source types: repair_order, invoice, b2b_order, b2b_return, sales_order, utility_practice, pos_transaction, warehouse_transfer, customer, supplier_order, other
- Storage CRUD methods with role-based filtering (reseller hierarchy, repair center isolation)
- API endpoints: GET `/api/reseller/documents`, GET `/api/repair-center/documents`, GET `/api/documents/:id`, GET `/api/documents/by-source/:sourceType/:sourceId`, DELETE `/api/documents/:id`
