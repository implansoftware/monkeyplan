...
import { ArrowLeft, Send, User, UserCog, Clock, AlertCircle } from "lucide-react";
...
export default function AdminTicketDetail() {
  ...
  // Extract ticketId from pathname using regex (robust against query strings/trailing slashes)
  const ticketIdMatch = location.match(/\/tickets\/([^/?]+)/);
  const ticketId = ticketIdMatch?.[1];
  
  // Determine base path for back navigation based on current location
  const getBasePath = () => {
    if (location.startsWith('/admin/')) return '/admin/tickets';
    if (location.startsWith('/reseller/')) return '/reseller/tickets';
    if (location.startsWith('/repair-center/')) return '/repair-center/tickets';
    if (location.startsWith('/customer/')) return '/customer/tickets';
    return '/tickets';
  };
  ...
  const updateStatusMutation = useMutation({
    ...
    onError: (error: Error) => {
      ...
      toast({ title: "Errore", description: serverMessage, variant: "destructive" });
    },
  });
  ...
  // Check if user can use internal notes (admin or assigned user)
  const canUseInternalNotes = isAdmin || ticket.assignedTo === user?.id;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => setLocation(getBasePath())}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  {messages.map((msg) => (
                    <div
                      ...
                      className={`border rounded-lg p-4 space-y-2 ${
                        msg.isInternal ? 'border-amber-500/50 bg-amber-500/5' : ''
                      }`}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="font-medium">
                            {msg.userId === ticket.customerId ? "Cliente" : "Staff"}
                          </span>
                          {msg.isInternal && (
        ...
        <div className="space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => 
                      assignTicketMutation.mutate(value === "unassigned" ? null : value)
                    }
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {/* TODO: Populate with actual staff users */}
                      <SelectItem value="admin">Admin</SelectItem>
                      <SelectItem value="support_team">Team Supporto</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Informazioni{isAdmin ? " Cliente" : ""}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              <div className="text-sm">
                <span className="text-muted-foreground">
                  {isAdmin ? "ID Cliente: " : "ID Ticket: "}
                </span>
                <span className="font-mono">
                  {isAdmin ? ticket.customerId : ticket.ticketNumber}
                </span>
              </div>
            </CardContent>
          </Card>
