...
type PosTransaction = {
  ...
  taxAmount: number;
  total: number;
  paymentMethod: "cash" | "card" | "pos_terminal" | "stripe_link" | "mixed";
  cashReceived: number | null;
  changeGiven: number | null;
  status: "completed" | "refunded" | "partial_refund" | "voided" | "pending" | "expired";
...
export default function ResellerPosTerminal() {
  ...
  // Parametri URL: /reseller/pos/terminal/:repairCenterId/:registerId
  const [match, params] = useRoute("/reseller/pos/terminal/:repairCenterId/:registerId");
  const repairCenterId = params?.repairCenterId || "";
  const urlRegisterId = params?.registerId || "";
  ...
  // Stato per Payment Link Stripe/PayPal
  ...
  const { data: registers = [], isLoading: registersLoading } = useQuery<PosRegister[]>({
    queryKey: ["/api/reseller/pos/registers", repairCenterId],
    queryFn: async () => {
      const res = await fetch(`/api/reseller/pos/registers?repairCenterId=${repairCenterId}`, { credentials: "include" });
      if (!res.ok) return [];
      return res.json();
    },
    enabled: !!repairCenterId,
  ...
  // Set register from URL or default when loaded
  useEffect(() => {
    if (urlRegisterId) {
      setSelectedRegisterId(urlRegisterId);
    } else if (registers.length > 0 && !selectedRegisterId) {
      const defaultReg = registers.find(r => r.isDefault) || registers[0];
      if (defaultReg) setSelectedRegisterId(defaultReg.id);
    }
  }, [registers, selectedRegisterId, urlRegisterId]);

  const selectedRegister = registers.find(r => r.id === selectedRegisterId);
  
  // Se non c'è repairCenterId, mostra errore
  if (!repairCenterId) {
    return (
  ...
  // Sessione corrente per questa cassa
  const { data: currentSession, isLoading: sessionLoading } = useQuery<PosSession | null>({
    queryKey: ["/api/reseller/pos/register", selectedRegisterId, "session"],
    queryFn: async () => {
      if (!selectedRegisterId) return null;
      const res = await fetch(`/api/reseller/pos/register/${selectedRegisterId}/session`, { credentials: "include" });
  ...
  // Transazioni della sessione corrente
  const { data: transactions = [], isLoading: transactionsLoading } = useQuery<PosTransaction[]>({
    queryKey: ["/api/reseller/pos", repairCenterId, "session-transactions", selectedRegisterId],
    queryFn: async () => {
      const res = await fetch(`/api/reseller/pos/${repairCenterId}/session-transactions?registerId=${selectedRegisterId}`, { credentials: "include" });
      if (!res.ok) return [];
      return res.json();
    },
  ...
  const { data: customers = [] } = useQuery<PosCustomerType[]>({
    ...
    enabled: customerType === "existing" && !!repairCenterId,
  });
  ...
  const openSessionMutation = useMutation({
    mutationFn: async (data: { openingCash: number; openingNotes?: string }) => {
      return apiRequest("POST", "/api/reseller/pos/session/open", { 
        ...data, 
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/reseller/pos/register", selectedRegisterId, "session"] });
  ...
  const closeSessionMutation = useMutation({
    mutationFn: async (data: { closingCash: number; closingNotes?: string }) => {
      return apiRequest("POST", `/api/reseller/pos/session/${currentSession?.id}/close`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/reseller/pos/register", selectedRegisterId, "session"] });
  ...
  const createTransactionMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await apiRequest("POST", "/api/reseller/pos/transaction", { 
        ...data, 
        registerId: selectedRegisterId,
        repairCenterId 
      });
  ...
  const createPaymentLinkMutation = useMutation({
    mutationFn: async (transactionId: string) => {
      const res = await apiRequest("POST", "/api/reseller/pos/create-payment-link", { transactionId });
      return res.json();
    },
  ...
  // Mutation per verificare stato pagamento
  const checkPaymentStatusMutation = useMutation({
    mutationFn: async (transactionId: string) => {
      const res = await apiRequest("GET", `/api/reseller/pos/check-payment-status/${transactionId}`);
      return res.json();
    },
  ...
  // Mutation per rigenerare link scaduto
  const regeneratePaymentLinkMutation = useMutation({
    mutationFn: async (transactionId: string) => {
      const res = await apiRequest("POST", "/api/reseller/pos/regenerate-payment-link", { transactionId });
      return res.json();
    },
    onSuccess: (data: { paymentUrl: string; sessionId: string; expiresAt: string }) => {
      setPaymentLinkUrl(data.paymentUrl);
  ...
  const createPaypalOrderMutation = useMutation({
    mutationFn: async (transactionId: string) => {
      const res = await apiRequest("POST", "/api/reseller/pos/create-paypal-order", { transactionId });
      return res.json();
    },
  ...
  const checkPaypalStatusMutation = useMutation({
    mutationFn: async (transactionId: string) => {
      const res = await apiRequest("GET", `/api/reseller/pos/check-paypal-status/${transactionId}`);
      return res.json();
    },
    onSuccess: (data: { status: string; paid: boolean }) => {
      if (data.paid) {
      ...
      <div className="flex flex-col items-center justify-center min-h-[80vh] p-4 gap-4">
        ...
        <Dialog open={openSessionDialog} onOpenChange={setOpenSessionDialog}>
          <DialogContent>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setOpenSessionDialog(false)}>{t("common.cancel")}</Button>
              <Button
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col lg:flex-row gap-4 p-4">
            ...
            <Tabs defaultValue="search" className="h-full flex flex-col">
              <TabsList className="mb-2 w-full justify-start gap-1 overflow-x-auto">
                ...
                <TabsTrigger value="warranties" data-testid="tab-warranties" className="text-xs sm:text-sm px-2 sm:px-3">
                  ...
                  <span className="hidden sm:inline">{t("warranties.title")}</span>
                </TabsTrigger>
                        ...
                        >
                          <div className="flex gap-2 mb-2">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-xs text-muted-foreground mt-0.5">{wp.durationMonths} {t("common.monthsLabel")}</div>
                            </div>
                          </div>
                  ...
                  {transactionsLoading ? (
                    ...
                    <div className="space-y-2">
                      {transactions.map((tx) => (
                        <button
                          key={tx.id}
                          onClick={() => navigate(`/reseller/pos/transaction/${tx.id}`)}
                    ...
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {item.isService && (
                            <Badge variant="outline" className="text-[10px] px-1 py-0 h-4 border-indigo-500 text-indigo-600 dark:text-indigo-400">{t("common.service")}</Badge>
                          )}
                          {item.isWarranty && (
                            <Badge variant="outline" className="text-[10px] px-1 py-0 h-4 border-emerald-500 text-emerald-600 dark:text-emerald-400">{t("common.warranty")}</Badge>
                          )}
                          {item.isTemporary && (
      ...
      <Dialog open={paymentDialog} onOpenChange={setPaymentDialog}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2 flex-wrap">
              ...
              <div className="ml-auto flex items-center gap-2">
                ...
                />
                <Label htmlFor="invoice-switch" className="text-xs">{t("common.invoice")}</Label>
              </div>
            </div>
          ...
          <DialogFooter className="gap-2 sm:gap-0 pt-2">
            <Button variant="outline" size="sm" onClick={() => setPaymentDialog(false)}>{t("common.cancel")}</Button>
            <Button
              size="sm"
            ...
            >
              ...
              <Check className="w-4 h-4 mr-2" />{t("common.confirm")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={closeSessionDialog} onOpenChange={setCloseSessionDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setCloseSessionDialog(false)}>{t("common.cancel")}</Button>
            <Button
              variant="destructive"
