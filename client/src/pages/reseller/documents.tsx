...
import { Search, FileText, Download, CalendarIcon, Eye, Trash2, FolderOpen, Receipt, Wrench, Truck, FileCheck, Tag, Filter } from "lucide-react";
...
export default function ResellerDocuments() {
  ...
  const { data, isLoading, refetch } = useQuery<{ documents: Document[]; total: number }>({
    queryKey: ["/api/reseller/documents", typeFilter, sourceFilter, searchQuery, dateRange?.from?.toISOString(), dateRange?.to?.toISOString()],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/reseller/documents?${params.toString()}`, {
        credentials: "include",
      });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/reseller/documents"] });
  ...
  return (
    <div className="space-y-6" data-testid="page-reseller-documents">
