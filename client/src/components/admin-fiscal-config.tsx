...
export function AdminFiscalConfig() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: t("fiscal.rtConfigSaved") });
    },
    onError: (error: any) => {
  ...
  const retryMutation = useMutation({
    ...
    onSuccess: (result: { success: boolean }) => {
      ...
      toast({
        title: result.success ? t("fiscal.retrySuccess") : t("fiscal.retryFailed"),
  ...
  const filterLabel = filterType === "all"
    ? t("common.allMasc")
    : filterType === "reseller"
      ? (resellers?.find(r => r.id === selectedResellerId)?.fullName || resellers?.find(r => r.id === selectedResellerId)?.username || t("common.select"))
      : (repairCentersList.find(rc => rc.id === selectedRepairCenterId)?.name || t("common.select"));
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            <Label data-testid="label-rt-provider">{t("fiscal.rtProviderEnabled")}</Label>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-0.5">
              <Label data-testid="label-sandbox-mode">{t("fiscal.sandboxMode")}</Label>
              <p className="text-sm text-muted-foreground">
                {t("fiscal.sandboxModeDesc")}
              </p>
            </div>
            <Switch
              checked={sandboxMode}
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle data-testid="text-rt-stats-title">{t("fiscal.rtStats")}</CardTitle>
              <CardDescription>{t("fiscal.rtStatsDesc")}</CardDescription>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="all" data-testid="option-filter-all">{t("common.allMasc")}</SelectItem>
                  <SelectItem value="reseller" data-testid="option-filter-reseller">{t("roles.reseller")}</SelectItem>
                  <SelectItem value="repair_center" data-testid="option-filter-rc">{t("roles.repairCenter")}</SelectItem>
                </SelectContent>
              </Select>
              ...
              {filterType === "reseller" && (
                <Select value={selectedResellerId} onValueChange={setSelectedResellerId}>
                  <SelectTrigger className="w-[200px]" data-testid="select-rt-filter-reseller">
                    <SelectValue placeholder={t("fiscal.selectReseller")} />
                  </SelectTrigger>
                  <SelectContent>
                    {resellers?.map((r: any) => (
              ...
              {filterType === "repair_center" && (
                <Select value={selectedRepairCenterId} onValueChange={setSelectedRepairCenterId}>
                  <SelectTrigger className="w-[200px]" data-testid="select-rt-filter-rc">
                    <SelectValue placeholder={t("fiscal.selectCenter")} />
                  </SelectTrigger>
                  <SelectContent>
        ...
        <CardContent>
          {!filterReady ? (
            <p className="text-sm text-muted-foreground">{t("fiscal.selectEntityForStats", { entity: filterType === "reseller" ? t("roles.reseller").toLowerCase() : t("roles.repairCenter").toLowerCase() })}</p>
          ) : statsLoading ? (
      ...
      {(failedTx && failedTx.length > 0) && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t("fiscal.failedTransmissionsDesc")}</CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {tx.rtRetryCount > 0 && (
                        <Badge variant="outline" data-testid={`badge-retry-count-${tx.id}`}>
                          {tx.rtRetryCount} {t("fiscal.attempts")}
                        </Badge>
                      )}
                    </div>
                    {tx.rtErrorMessage && (
